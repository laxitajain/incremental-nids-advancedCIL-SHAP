...
def get_data(full_path, num_tasks, nc_first_task, nc_incr_tasks, validation, shuffle_classes, 
    ...
    # We skip prepending '../' because it breaks when only pre-processed data exists
    ...
    labels = np.loadtxt(os.path.join(path, f"classes_{dataset_filename.split('.')[0]}.txt"), dtype=str)
