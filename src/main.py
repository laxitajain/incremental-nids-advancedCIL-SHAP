...
def main(argv=None):
    ...
    parser.add_argument('--shap-weights', default=None, type=str, required=False,
                        help='Path to SHAP weights .npy file to initialize ShapAttention (default=%(default)s)')
    # training args
    ...
    shap_weights = None
    if args.shap_weights is not None and os.path.exists(args.shap_weights):
        print(f"Loading SHAP weights from {args.shap_weights}")
        shap_weights = np.load(args.shap_weights)

    net = getattr(importlib.import_module(name='networks'), args.network)
    init_model = net(num_pkts=args.num_pkts, num_fields=len(args.fields), shap_weights=shap_weights)
