...
class Lopez17CNN(nn.Module):
    ...
    def __init__(self, in_channels=1, num_classes=10, **kwargs):
        ...
        # Optional SHAP Attention
        self.shap_weights = kwargs.get('shap_weights', None)
        if self.shap_weights is not None:
            from .shap_attention import ShapAttention
            self.shap_attention = ShapAttention(num_fields, shap_weights=self.shap_weights)

    def forward(self, x):
        if self.shap_weights is not None:
            x = self.shap_attention(x)
